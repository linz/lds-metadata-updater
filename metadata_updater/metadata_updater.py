    ...
    def __init__(self, cwd=None):
        ...
        if "Test" in config:
            ...
            if self.test_dry_run not in (True, False):
                raise SystemExit(
                    'CONFIG ERROR: "Test Dry Run" must be '
            ...
            if self.test_overwrite not in (True, False):
                raise SystemExit(
                    'CONFIG ERROR: "test_overwrite" must be '
...
def post_metadata(draft, file):
    ...
    try:
        ...
        ERRORS += 1
        logger.critical(
            "metadata update for {0} fail with {1}".format(draft.id, str(e))
        )
        return False
...
def get_metadata(layer, dir, overwrite):
    ...
    file_destination = os.path.join(
        dir, "{0}_{1}_{2}.iso.xml".format(layer.type, layer.id, title)
    )
...
def delete_draft(layer, version):
    ...
    try:
        ...
        logger.info(
            "A draft already exists for {0}. This draft "
...
def get_draft(layer):
    ...
    if hasattr(draft, "active_publish"):
        if draft.active_publish:
            ...
            ERRORS += 1
            logger.critical(
                "A draft already exists for {0} and is in a "
...
def parse_args(args):
    ...
    cli_parser.add_argument(
        "--config_file", default=None, nargs="?", help="Path to config file"
    )
...
def main():
    ...
    for layer_id in layer_ids:
        ...
        if not layer:
            ERRORS += 1
            logger.critical(
                "Failed to get layer {0}. THIS LAYER HAS NOT BEEN PROCESSED".format(
                    layer_id
                )
            )
            continue
        ...
        if not file:
            ERRORS += 1
            logger.critical(
                "Failed to get metadata from layer {0}. THIS LAYER HAS NOT BEEN PROCESSED".format(
                    layer_id
                )
            )
            continue
    ...
    if layers_edited_count > 0 and not config.test_dry_run:
        try:
            ...
            logger.info(
                "{0} layer(s) processed | {1} layer(s) edited".format(
                    layer_count, layers_edited_count
                )
            )
        except koordinates.exceptions.ServerError as e:
    ...
    if ERRORS > 0:
        ...
        print(
            "Process failed with {0} error(s). Please see log for critical messages".format(
                ERRORS
            )
        )
