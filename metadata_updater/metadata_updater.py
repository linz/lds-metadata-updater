    ...
    def __init__(self, cwd=None):
        ...
        if "Test" in config:
            ...
            if self.test_dry_run not in (True, False):
                raise SystemExit(
                    'CONFIG ERROR: "Test Dry Run" must be ' '"True" or "False". Got:"{}" instead'.format(self.test_dry_run)
                )

            self.test_overwrite = config["Test"]["Overwrite_files"]
            if self.test_overwrite not in (True, False):
                raise SystemExit(
                    'CONFIG ERROR: "test_overwrite" must be ' '"True" or "False". Got:"{}" instead'.format(self.test_overwrite)
                )
        else:
...
def post_metadata(draft, file):
    ...
    try:
        ...
        ERRORS += 1
        logger.critical("metadata update for {0} fail with {1}".format(draft.id, str(e)))
        return False
...
def get_metadata(layer, dir, overwrite):
    ...
    file_destination = os.path.join(dir, "{0}_{1}_{2}.iso.xml".format(layer.type, layer.id, title))
...
def delete_draft(layer, version):
    ...
    try:
        ...
        logger.info("A draft already exists for {0}. This draft " "was deleted and a new one created ".format(layer.id))
        return True
    except koordinates.exceptions.ServerError as e:
...
def get_draft(layer):
    ...
    if hasattr(draft, "active_publish"):
        if draft.active_publish:
            ...
            ERRORS += 1
            logger.critical(
                "A draft already exists for {0} and is in a " "publish group. THIS HAS NOT BEEN UPDATED ".format(layer.id)
            )
            return None
        else:
    ...
    *Currently only layers and tables are handled
    ...
    for item in client.catalog.list():
        if type(item) == type(koordinates.layers.Layer()):
            ...
            try:
                logger.warning(
                    'Dataset {0}: Data is of "{1}" type. \
                This process only handles tables/layers'.format(
                        item.id, type(item)
                    )
                )
            except:
...
def main():
    ...
    if layers_edited_count > 0 and not config.test_dry_run:
        try:
            ...
            logger.info("{0} layer(s) processed | {1} layer(s) edited".format(layer_count, layers_edited_count))
        except koordinates.exceptions.ServerError as e:
